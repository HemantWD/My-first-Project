    ...
    return (
        <>
            <div className='container' style={{ color: props.mode === 'dark' ? 'white' : 'black' }}>
                ...
                <button className="btn btn-primary m-3" onClick={handleExtraSpace}>Remove Extra Spaces</button>
                <button className="btn btn-primary m-3" onClick={handleClearClick}>Clear</button>
            </div>
            <div className="containe3 my-2" style={{ color: props.mode === 'dark' ? 'white' : 'black' }}>
                <h2>Your Text Summary</h2>
                <p>{text.split(" ").length} words and {text.length} characters</p>
                {/* Below is How much time spend in reading the text */}
                <p>{0.008 * text.split(" ").length} Minutes took to read it</p>
                {/* To preview text */}
                <h3>Preview</h3>
